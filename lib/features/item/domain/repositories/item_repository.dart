...
class ItemRepository implements ItemRepositoryInterface {
  ...
  @override
  Future get(String? id, {bool isConditionWiseItem = false, bool isCampaign = false}) async {
  ...
  Future<Item?> _getItemDetails(int? itemID, bool isCampaign) async {
    Item? item;
    Response response = await apiClient.getData('${AppConstants.itemDetailsUri}$itemID${isCampaign ? '?campaign=1' : ''}');
    if (response.statusCode == 200) {
